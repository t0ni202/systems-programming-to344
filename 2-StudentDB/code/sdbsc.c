...
int get_student(int fd, int id, student_t *s){
	...
	// CALCULATE OFFSET FOR THIS STUDENT ID
	...
	// CHECK IF RECORD EXISTS (NOT ALL ZEROS)
...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
	student_t new_student = {0}; //intialize to 0
	student_t existing = {0};

	// calculate offset for this student ID
	off_t offset = id * STUDENT_RECORD_SIZE;

	// FIRST CHECK IF A STUDENT ALREADY EXISTS IN THIS POSITION
	if (lseek(fd, offset, SEEK_SET) == -1){
		printf(M_ERR_DB_READ);
		return ERR_DB_FILE;
	}

	// READ WHATS AT THIS POSITION
	if (read(fd, &existing, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE){
		// IF THE RECORD EXISTS (NOT ALL 0s), STUDENT ID IS TAKEN
	...
	// seek back to the position to write
	if (lseek(fd, offset, SEEK_SET) == -1){
...
int del_student(int fd, int id){
	...
	// calculate offset fir this student id
	off_t offset = id * STUDENT_RECORD_SIZE;
...
int print_db(int fd){
	...
	while((bytes_read = read(fd, &student, STUDENT_RECORD_SIZE)) == STUDENT_RECORD_SIZE){
		...
		//print the header before the first valid record
		if (!header_printed){
			printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST NAME", "LAST_NAME", "GPA");
			header_printed = true; //flag
		}

		// Calculate real GPA nad print record
		float real_gpa = student.gpa / 100.0;
		printf(STUDENT_PRINT_FMT_STRING, student.id, student.fname, student.lname, real_gpa);

	}

	// Check if we are at the EOF or had an error 
	if (bytes_read != 0 && bytes_read != STUDENT_RECORD_SIZE){
		printf(M_ERR_DB_READ);
		return ERR_DB_FILE;
	}

	// if no recrods were found, print empty message
	if (!header_printed){
		printf(M_DB_EMPTY);
	}
...
void print_student(student_t *s){
	// vslidate the student pointer and ID
	if (s == NULL || s->id == 0){
	...
	// convert integer GPA to float (gpa is stored as int *100)
...
int compress_db(int fd){
	...
	//open temporary file
	tmp_fd = open_db(TMP_DB_FILE, true); //to truncate/create new
	if(tmp_fd < 0){
		printf(M_ERR_DB_OPEN);
		return ERR_DB_FILE;
	}
	...
	while ((bytes_read = read(fd, &student, STUDENT_RECORD_SIZE)) == STUDENT_RECORD_SIZE){
		...
		//Calculate correct position in the new file
		off_t new_offset = student.id * STUDENT_RECORD_SIZE;

		//Seek to correct position in the temp file
		if (lseek(tmp_fd, new_offset, SEEK_SET) == -1){
			printf(M_ERR_DB_WRITE);
			close(tmp_fd);
			return ERR_DB_FILE;
		}

		//Write record tp new position
